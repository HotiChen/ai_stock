...
def _render_strategy_tracker():
    ...
    def _build_candidates() -> list[dict]:
        """從 ai_log 抓候選股票 + 隨機 20 支主題股，去重補價，並附上籌碼資料。"""
        import random
        ...
        # ── 5. 為每檔候選股附上籌碼標注，並計算排序分數 ──────────
        for c in deduped:
            code = c["code"]
            chip_parts: list[str] = []
            chip_score = 0  # 分數越高越優先
            ...
            if inst:
                ...
                if foreign_net != 0:
                    ...
                    chip_score += min(foreign_net / 500, 5)   # 每500張得1分，最多5分
                if trust_net != 0:
                    chip_parts.append(f"投信{'+' if trust_net > 0 else ''}{trust_net:.0f}張")
                    chip_score += min(trust_net / 100, 3)     # 每100張得1分，最多3分
                if total_net > 0:
                    chip_score += 1  # 三大法人合計買超加分
            ...
            if margin:
                ...
                if margin_prev > 0:
                    ...
                    if abs(margin_pct) >= 3:
                        direction = "↑" if margin_pct > 0 else "↓"
                        chip_parts.append(f"融資{direction}{abs(margin_pct):.1f}%")
                    if margin_pct > 10:
                        chip_score -= 2   # 融資暴增，風險扣分
        ...
        # ── 6. 依籌碼分數重排（分數高的優先給 AI 看）────────────────
        deduped.sort(key=lambda x: x.get("chip_score", 0), reverse=True)

        n_chip  = len([c for c in deduped if c.get("chip_score", 0) > 0])
        n_theme = len([c for c in deduped if c.get("analysis", "").startswith("主題池")])
        print(f"[_build_candidates] 共 {len(deduped)} 支｜有籌碼={n_chip}｜主題補充={n_theme}", flush=True)
        return deduped
